...
def custom_collate_fn(batch):
    # Extract keys from the first item to know the structure
    keys = batch[0].keys()
    ...
    batched['attention_mask'] = torch.nn.utils.rnn.pad_sequence(batched['attention_mask'], batch_first=True,
                                                                padding_value=0)
    batched['pixel_values'] = torch.stack(batched['pixel_values'])  # Pixel values should already be uniform
...
# Save the model
model.save_pretrained("git_caption_model")
processor.save_pretrained("git_caption_model")
print("Model saved as 'git_caption_model'")
